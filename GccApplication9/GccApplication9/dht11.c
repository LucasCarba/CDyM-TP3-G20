...
// paso 5 leemos los 40 bits o 5 bytes los dos primero bytes, corresponde a la humedad 3 y 4 a la temperatura
	for (j=0; j<5; j++)
	{
		...
		for(i=0; i<8;i++){  // Leer 8 bits de datos del sensor DHT11
			while(!(DHT_PIN & (1<<PIN)))
			_delay_us(35); // Retraso para esperar el inicio de la transmisión
			if(DHT_PIN & (1<<PIN))   // Esperar a que el pin del sensor esté alto (datos)
		...
		DHT_PORT |= (1<<PIN); // Establecer el nivel lógico alto en el pin del sensor DHT11
